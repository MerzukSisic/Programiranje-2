...
class Kupovina {
    ...
    void SaljiMail(ZadovoljstvoKupca kupac) {
        ...
        cout << "Postovani,\nZaprimili smo Vase ocjene, a njihova prosjecna vrijednost je " << kupac.Prosjek() << endl;
...
public:
    ...
    void SetZadovoljstvoKupca(ZadovoljstvoKupca z) {
        ...
        int brojac = 0;
        for (int i = 0; i < z.GetOcjeneKarakteristika()->getTrenutno(); i++)
            if (z.GetOcjeneKarakteristika()->getElement2(i) < 5)
                brojac++;
        if (brojac >= 2) {
            thread t1(&Kupovina::SaljiMail, this, z);
            t1.join();
        }
